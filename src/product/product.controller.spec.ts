...
describe('ProductController', () => {
  ...
  const mockProductRepository = {
    ...
    createProduct: jest.fn((createProductDto: CreateProductDto) => {
      // Simulate cloudinary upload success
      if (createProductDto.images === 'test.png') {
        return { ...createProductDto, idProduct: 15 }; // Simulate returning the created product
      } else {
  ...
  describe('Testing over create method', () => {
    it('should create a new product', async () => {
      ...
      const productReal = await productController.create(
        ...
        newProduct.category,
        newProduct.amount,
        true,  // Assuming the active status is true
        mockFile
      );
